...
contract('NeverPayShares', (accounts) => {
    ...
    it("Test that an investor may successfully transfer shares that they are approved by another investor", async () => {
        ...
        assert.equal(myAllowances.words[0], 250, "Allowance of 250 shares was not approved from 1st investor to 2nd investor");

        await NeverPayShares.transferFrom(accounts[2], accounts[4], 150, {from: accounts[3]});
        let investor3Balance = await NeverPayShares.balanceOf(accounts[4], {from: accounts[4]});
        let investor1Balance = await NeverPayShares.balanceOf(accounts[2], {from: accounts[2]});

        assert.equal(investor3Balance.words[0], 650, "Investor did not receive an approved amount of +150 shares")
        assert.equal(investor1Balance.words[0], 350, "Shares were not reduced from given allowance");

        let myNewAllowances = await NeverPayShares.allowance.call(accounts[2], accounts[3], {from: accounts[2]});
        assert.equal(myNewAllowances.words[0], 100, "Initial allowance of 250 was not reduced by 150 to equal 100");
    })  
})