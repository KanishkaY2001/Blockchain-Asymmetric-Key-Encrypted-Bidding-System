...
contract('NeverPayShares', (accounts) => {
    ...
    it("Test that when all shares are sold, the lowest valid bidder gets leftovers", async () => {
        ...
        for (let numberOfInvestors = 0; numberOfInvestors < totalInvestors; ++numberOfInvestors) {
            ...
            let sharesInvestorWanted = investorBidInformation.shares;
            ...
            if (numberOfInvestors > 3) {
                assert.equal(sharesBalance.words[0], sharesInvestorWanted, "Investor did not receive all the shares they bought");
            } else if (numberOfInvestors == 3) {
                assert.equal(sharesBalance.words[0], 10000 - (1500 * 6), "Investor did not receive the appropriate leftover shares");
            } else {
