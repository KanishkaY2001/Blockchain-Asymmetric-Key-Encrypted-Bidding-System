...
contract('NeverPayShares', (accounts) => {
    ...
    it("Test that the bidder lower than lowest valid bid cannot claim shares", async () => {
        ...
        let validBidder = investorBidInformationArray[0];
        let validBidHash = validBidder.hash;
        let validInvestor = accounts[2];
        await NeverPayShares.claimShares(validBidHash, {from: validInvestor, gas: 6721975});

        let sharesBalance = await NeverPayShares.balanceOf(validInvestor, {from: validInvestor});
        assert.equal(sharesBalance.words[0], 0, "Investor received shares, which is not meant to happen, because they are unsuccessful");
    })
})