...
contract('NeverPayShares', (accounts) => {
    ...
    it("Test that despite higher demand, only 10000 shares max may be sold", async () => {
        ...
        for (let numberOfInvestors = 0; numberOfInvestors < totalInvestors; ++numberOfInvestors) {
            ...
            let investorBidInformation = InvestorLogic.getBidHash(1100, 1 + numberOfInvestors);
        ...
        assert.equal(totalSupply.words[0], 10000, "Demand: 11000 shares, sold more shares than max supply")
    })
})