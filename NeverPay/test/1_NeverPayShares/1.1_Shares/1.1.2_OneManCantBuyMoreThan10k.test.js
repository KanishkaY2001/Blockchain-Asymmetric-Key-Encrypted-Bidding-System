...
contract('NeverPayShares', (accounts) => {
    ...
    let ASIC = accounts[1];
    let totalInvestors = 1;
    let signerAuthorityAccount = CertificationLogic.signerAuthorityAccount;
    
    it("Test that an investor may only purchase a maximum of 10000 bids, despite paying for more", async () => {
        ...
        for (let numberOfInvestors = 0; numberOfInvestors < totalInvestors; ++numberOfInvestors) {
            ...
            let investorBidInformation = InvestorLogic.getBidHash(14001, 1 + numberOfInvestors);
        ...
        for (let numberOfInvestors = 0; numberOfInvestors < totalInvestors; ++numberOfInvestors) {
            ...
            assert.equal(sharesBalance.words[0], 10000, "Demand > 10k share supply, yet the investor did not get this amount")
        }

        let totalSupply = await NeverPayShares.totalSupply({from: accounts[2]});
        assert.equal(totalSupply.words[0], 10000, "Demand: 14001 shares, sold more or less shares than max supply")
    })
})