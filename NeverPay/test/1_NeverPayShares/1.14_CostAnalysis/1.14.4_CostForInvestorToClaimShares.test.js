...
contract('NeverPayShares', (accounts) => {
    ...
    it("Test that the lowest valid bidder gets all shares they bought if enough supply is available", async () => {
        ...
        for (let numberOfInvestors = 0; numberOfInvestors < totalInvestors; ++numberOfInvestors) {
            ...
            let investorBidInformation = InvestorLogic.getBidHash(200, 1 + numberOfInvestors);
        ...
        for (let numberOfInvestors = 0; numberOfInvestors < totalInvestors; ++numberOfInvestors) {
            ...
            const receipt = await NeverPayShares.claimShares(validBidHash, {from: currentInvestor, gas: 6721975});
