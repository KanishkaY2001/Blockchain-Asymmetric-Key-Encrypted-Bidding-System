...
contract('NeverPayShares', (accounts) => {
    ...
    it("Test that the information that the investor reveals cannot be different from their initial bid", async () => {
        ...
        let fabricatedInformation = InvestorLogic.getBidHash(200, 5);
        let fabricatedShares = fabricatedInformation.shares;
        let fabricatedPrice = fabricatedInformation.price;

        await truffleAssert.reverts(NeverPayShares.purchaseShares(fabricatedShares, fabricatedPrice, nonce, nonce, {from: investor, value: valueOfSharesInWei}));
    })
})