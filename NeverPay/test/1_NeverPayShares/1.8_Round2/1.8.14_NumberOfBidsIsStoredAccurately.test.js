...
contract('NeverPayShares', (accounts) => {
    ...
    it("Test that the variable that represents bids size is represented accurately by total number of bids", async () => {
        ...
        let numOfBids = await NeverPayShares.paidBidCount({from: accounts[2]});
        assert.equal(numOfBids, totalInvestors, "Number of bids in the list is not reflected by the total count that is publically viewable")
    })
})