...
contract('NeverPayShares', (accounts) => {
    ...
    let ASIC = accounts[1];
    let totalInvestors = 4;
    let signerAuthorityAccount = CertificationLogic.signerAuthorityAccount;
    
    it("Test that a lower bid, than current head node, will become the new lowest bid (head node)", async () => {
        ...
        for (let numberOfInvestors = 0; numberOfInvestors < totalInvestors; ++numberOfInvestors) {
            ...
            let investorBidInformation = InvestorLogic.getBidHash(10, 2 + numberOfInvestors);
        ...
        certificateSignature = CertificationLogic.createCertificate(accounts[6]).signature;
        newLowestBidderInformation = InvestorLogic.getBidHash(10, 1);
        bidHash = newLowestBidderInformation.hash;
        await NeverPayShares.bid(bidHash, 0, certificateSignature, {from: accounts[6]});
        ...
        let currentInvestor = accounts[6];
        let previousLowestBidHash = await NeverPayShares.firstBid({from: accounts[6]});
        
        let nonce = newLowestBidderInformation.nonce;
        let newLowestBidHash = newLowestBidderInformation.hash;
        let valueOfSharesInWei = newLowestBidderInformation.valueOfSharesInWei;

        let bidHint = await bidHintFindingHelper(newLowestBidderInformation, currentInvestor);
        await NeverPayShares.purchaseShares(10, 1, nonce, bidHint, {from: currentInvestor, value: valueOfSharesInWei, gas: 6721975});
        
        let roundTwoData = await NeverPayShares.roundTwoData(newLowestBidHash, {from: currentInvestor});

        assert.equal(roundTwoData.next, previousLowestBidHash, "This bid should be the new lowest, yet it has no reference to the previous lowest");
        assert.equal(roundTwoData.prior, 0, "There are bids lower than the lowest, which makes no sense");
    })
})