...
contract('NeverPayShares', (accounts) => {
    ...
    it("Test that, if only one bid exists, it is equal to the tail and head node", async () => {
        ...
        let firstBid = await NeverPayShares.firstBid.call({from: investor});
        let lastBid = await NeverPayShares.lastBid.call({from: investor});
        assert.equal(firstBid, lastBid, "Tail and head nodes are not equal to eachother, even with a single bid")
    })
})