...
contract('NeverPayShares', (accounts) => {
    ...
    it("Test that the investor's bid data is revealed in round 2", async () => {
        ...
        let bidInformation = await NeverPayShares.roundTwoData.call(bidHash, {from: investor});
        assert.equal(bidInformation.shares, shares, "Shares information (shares) is not revealed");
        assert.equal(bidInformation.priceEth, price, "Shares information (price per share) is not revealed");
    })
})