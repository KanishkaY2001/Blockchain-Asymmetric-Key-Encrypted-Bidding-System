...
contract('NeverPayShares', (accounts) => {
    ...
    it("Test that the linked list works as intended, by ensuring that there is no lower bid than head node", async () => {
        ...
        let headNode = await NeverPayShares.firstBid({from: accounts[2]});
        let roundTwoData = await NeverPayShares.roundTwoData.call(headNode, {from: accounts[2]});
        assert.equal(roundTwoData.prior, 0, "Linked list does not work, because the head node is not the 'true' head (lowest) of the list")
    })
})